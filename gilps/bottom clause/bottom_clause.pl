...
% YAP modules
:- use_module(library(rbtrees), [rb_empty/1, rb_lookup/3, rb_insert/4, rb_visit/2, rb_update/4, rb_update/5]).
...
show_bottom_clause(ExampleID, Mode):-
  ...
  format('~2n Bottom clause for example id ~w, ~k:~n', [ExampleID, Example]),
  ...
  format('[Num literals=~w]', [NumLiterals]).
